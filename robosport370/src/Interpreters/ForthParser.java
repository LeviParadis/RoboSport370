...
public class ForthParser {
    ...
    /**
     * Parses text into a sequence of forth word objects. Will throw an
     * exception if it encounters a word it can't parse
     * 
     * @param logicString
     *            the string we want to convert into forth code
     * @param robot
     *            the robot that is executing the program
            ...
            if (item.length() > 1 && item.charAt(0) == '.' && item.charAt(1) == '"'
                ...
                while (!stringEnd) {
                    ...
                    if (i >= elements.length) {
                        // we have reached the end of all words without finding
                        // the closing "
     ...
     * 
     * @param wordString
     *            a queue of plain text words
     * @param iterator
     *            used to keep track of our place in the list, so the method can
     *            be called recursively but retain placement
     * @param robot
     *            the robot that is running the program
     * @param expectedEnding
     *            if the function is called recursively, you can give an ending
     *            word that will cause the function to return immediately
     * @return a queue of forth words that can be run by the forth interpreter
     * @throws ForthParseException
     *             thrown if the forth text cannot be parsed
     */
    private static Queue<ForthWord> createWordList(Queue<String> wordString, Iterator<String> iterator, Robot robot,
            String expectedEnding) throws ForthParseException {
        ...
        while (iterator.hasNext()) {
            ...
            if (item.equals(IF_START_WORD)) {
                // if we encounter an if statement, call the function
                // recursively, and stop when we reach then
                // everything in between if and then will be collapsed into an
                // if object
                ...
                // if we reach the designated ending word (then, loop, until,
                // etc) return
                // what we have collected so far immediately
                return commandQueue;
            } else {
                // if it is not a control flow word, use a seperate function to
                // determine what forth word it is
                ForthWord newWord = wordFromString(item, robot);
                commandQueue.add(newWord);
            }
        }
        if (expectedEnding != null) {
            // if we reach the end of the words without reaching the expected
            // ending, something
     ...
     * @param robot
     *            the robot that executed this code
