...
public class Map {
    ...
    public Map() {
        ...
        for (int left = 0; left < mapDiameter; left++) {
            ...
            for (int y = 0; y < height; y++) {
                ...
                if (test) {
                    System.out.print("(" + temp.getXCoord() + "," + temp.getYCoord() + ")" + temp.getCost());
                }
    ...
    public int chooseIndex(int i, int j) {
        ...
        // But makes it so the map isn't either random or just all of one type
        // of tile
        if (i == 9 || i == 12 || (i == 3 && j > 7)) {
            return 1;
        }
    ...
    public enum DIRECTION {
        NORTH(0, 1), SOUTH(0, -1), NORTH_WEST(-1, 0), NORTH_EAST(1, 1), SOUTH_WEST(-1, -1), SOUTH_EAST(1, 0);
