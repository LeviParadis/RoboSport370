...
public class ForthSystemWord implements ForthWord {
     ...
     * 
     * @param wordName
     *            the plain text version of the word
     * @throws ForthParseException
     *             if the input word does not match a system command
     */
    public ForthSystemWord(String wordName) throws ForthParseException {
        ...
        if (type == SystemCommandType.NOT_FORTH_WORD) {
            throw new ForthParseException(
                    "attempted to instantiate a forth system command with unknown word " + wordName);
        }
        this.kind = type;
        this.name = wordName;
    }
